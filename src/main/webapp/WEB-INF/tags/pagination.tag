	...
	<ul class="pagination justify-content-center">
		<c:choose>
			<c:when test="${pageNumber gt 1}">
				<li class="page-item"><a class="page-link link-dark"
					href="${jsp}&pageNumber=${pageNumber - 1}">Previous</a></li>
			</c:when>
			<c:otherwise>
				<li class="page-item disabled"><a class="page-link link-dark" href="#"
		...
		<c:forEach begin="1" end="${numberOfPages}" var="page">
			<c:choose>
				<c:when test="${page != pageNumber}">
					<li class="page-item"><a class="page-link link-dark"
						href="${jsp}&pageNumber=${page}">${page}</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item active" aria-current="page"><a class="page-link link-dark" href="#"
		...
		<c:choose>
			<c:when test="${pageNumber lt numberOfPages}">
				<li class="page-item"><a class="page-link link-dark"
					href="${jsp}&pageNumber=${pageNumber + 1}">Next</a></li>
			</c:when>
			<c:otherwise>
				<li class="page-item disabled"><a class="page-link link-dark" href="#"
